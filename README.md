...
- **Full Validation** — Reproduce all paper statistics using the Jupyter notebook (60 min)
- **Additional Validation** — Test the MicroREF data collection pipeline with synthetic examples (optional)
...
**Note:** This contains repository *metadata* only, not the actual code. To test the complete pipeline including code analysis, manually clone any of the referenced repositories (e.g., `git clone https://github.com/github/gitignore.git .generated/microref/out/repositories/github_gitignore`). This design keeps the artifact small while remaining fully reproducible.
